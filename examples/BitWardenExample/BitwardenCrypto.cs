...
namespace BitWardenExample;

/// <summary>
/// Minimal, self-contained implementation of Bitwarden's client-side key derivation and
/// EncString decryption. Kept in the example project so the decrypt chain can be proven against a
/// real server before being ported into the adapter.
///
/// Key hierarchy:
...
public static class BitwardenCrypto
{
    ...
    public static string DecryptSymmetricToString(string encString, byte[] combined64)
        => DecryptSymmetricToString(encString, combined64[..32], combined64[32..64]);
